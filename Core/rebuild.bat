...
IF %DEBUG%==1 (
	...
	"%DEVENV_ROOT%\devenv.exe" "%RUBBERBAND_ENGINE_CORE%\RubberbandEngineCore.vcxproj" %TYPE% "Debug|x64" /Out buildLog_Debug.txt
)

IF %RELEASE%==1 (
	ECHO %TYPE_NAME% RELEASE
	"%DEVENV_ROOT%\devenv.exe" "%RUBBERBAND_ENGINE_CORE%\RubberbandEngineCore.vcxproj" %TYPE% "Release|x64" /Out buildLog_Release.txt
) 
